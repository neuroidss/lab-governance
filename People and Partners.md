...
The full-time compensation level for a Partner may only be set at `80% + (glue value) - (investment)` of the `personal market comp benchmark` or the pro-rated equivalent for a part-time commitment. The `personal market comp benchmark` is the highest pay level that matches a set of Badges the Partner holds. `glue value` may only be between 0 and 20%. In the normal case the `investment` is 20%.
